...
## 14. Seguridad de secretos
- ANTES de sugerir subir codigo a internet (git, etc): revisa que no haya contraseñas, API keys, o tokens en texto plano.
- Prefiere el uso de variables de entorno o archivos `.env` (asegurandote de que esten en `.gitignore`).
## 15. Ejecución proactiva
- Usa `SafeToAutoRun: true` en todos los comandos que no sean destructivos (borrado de archivos, cambios irreversibles) para evitar interrupciones innecesarias.
- No esperes aprobación para tareas rutinarias de validación o lectura.
